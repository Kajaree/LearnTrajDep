...
from utils.arDataset import ARDataset
...
def split_dataset(dataset, stepsize=10, maxsize=20):
    iteration = maxsize % stepsize
    ar_datasets = []
    start = 0
...
def main(opt):
    ...
    for epoch in range(start_epoch, opt.epochs):
        ...
        head = np.array(['epoch'])
        # per epoch
        lr_now, t_l, t_e, t_3d = train(train_dataset, train_loader, model, optimizer, input_n=input_n,
...
def train(train_dataset, train_loader, model, optimizer, input_n=20, dct_n=20, lr_now=None, max_norm=True, is_cuda=False, dim_used=[]):
    train_batch = 16
    job = 0
    t_l = utils.AccumLoss()
    t_e = utils.AccumLoss()
    t_3d = utils.AccumLoss()
    ar_datasets = split_dataset(train_dataset)
    model.train()
    st = time.time()
    ...
    for i, (inputs, targets, all_seq) in enumerate(train_loader):
        ...
        bt = time.time()
        outputs = None
        losses, m_errs, e_errs = [], [], []
        for ar_dataset in ar_datasets:
            ...
            ar_train_loader = DataLoader(
                ...
                num_workers=job,
                pin_memory=True)
            for index, (ar_inputs, ar_targets, ar_all_seq) in enumerate(ar_train_loader):
                if outputs is not None:
                    ar_inputs = torch.cat(outputs, ar_inputs)
                if is_cuda:
                    ar_inputs = Variable(ar_inputs.cuda()).float()
                    # targets = Variable(targets.cuda(async=True)).float()
                    ar_all_seq = Variable(ar_all_seq.cuda(async=True)).float()
